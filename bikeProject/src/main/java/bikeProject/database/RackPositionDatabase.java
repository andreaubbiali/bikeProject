...
public class RackPositionDatabase implements RackPositionDatabaseInterface {
    ...
    public void updateIsBroken(long ID, boolean isBroken) throws SQLException {
        ...
        PreparedStatement statement =
                Database.getConn().prepareStatement("UPDATE rack_position SET is_broken = ? " + "WHERE id = ?; ");
    ...
    public void setBikeNull(long rackID) throws SQLException {

        // prepare the statement
        PreparedStatement statement =
                Database.getConn().prepareStatement("UPDATE rack_position SET bike_id = null " + "WHERE id = ?");
        statement.setObject(1, rackID);

        // execute the query
        int res = statement.executeUpdate();
        if ( res == 0 ) {
            throw new SQLException("Set bike to null failed, no rows affected.");
        }
    }
}